...
.av{width:38px;height:38px;background:#1D9E75;color:#fff;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:12px;flex-shrink:0}
.logo strong{font-size:15px;display:block}
...
.tab{padding:10px 20px;border:none;background:none;font-size:15px;font-weight:500;color:#666;cursor:pointer;border-bottom:3px solid transparent;margin-bottom:-2px;transition:all .15s}
...
.cl{display:block;padding:8px 16px;font-size:13px;color:#444;border-left:3px solid transparent;transition:all .12s}
.cl:hover,.cl.on{background:#f0faf6;color:#1D9E75;border-color:#1D9E75}
.ia{flex:1}
.cs{margin-bottom:36px;scroll-margin-top:80px}
...
.ic:hover{box-shadow:0 4px 18px rgba(0,0,0,.1);transform:translateY(-2px)}
.ic.ft{border-color:#1D9E75}
...
.ib{padding:14px 14px 8px;flex:1}
.in{font-size:14px;font-weight:600;color:#111;line-height:1.35}
...
.ip{font-size:13px;font-weight:600;color:#1D9E75}
.io{font-size:12px;color:#999}
...
function showTab(i){
  ...
  document.querySelectorAll('.pnl').forEach((p,idx)=>p.classList.toggle('on',idx===i));
}
const obs=new IntersectionObserver(ee=>ee.forEach(e=>{if(e.isIntersecting){document.querySelectorAll('.cl').forEach(l=>l.classList.remove('on'));const t=document.querySelector('.cl[href="#'+e.target.id+'"]');if(t)t.classList.add('on');}}),{threshold:0.3});
document.querySelectorAll('.cs').forEach(s=>obs.observe(s));
</script>
</body>
</html>
