...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $bid    = $_POST['bid']    ?? '';
    if ($action === 'remove') {
        $_SESSION['basket'] = array_values(
            array_filter($_SESSION['basket'] ?? [], fn($b) => $b['id'] !== $bid)
        );
    } elseif ($action === 'inc' || $action === 'dec') {
        foreach ($_SESSION['basket'] as &$b) {
            if ($b['id'] === $bid) {
                $b['qty'] = max(1, ($b['qty'] ?? 1) + ($action === 'inc' ? 1 : -1));
                break;
            }
        }
    }
...
<head>
  ...
  <title>Basket — Cedar Grove</title>
...
<main class="container basket-page">
  ...
  <div class="basket-layout">
      ...
      <div class="basket-card">
        <div class="basket-card__top">
          <div>
            ...
            <?php if ($b['size_label']): ?>
              <p class="basket-card__size"><?= esc($b['size_label']) ?></p>
            <?php endif; ?>
            <?php foreach ($b['selections'] as $sel): ?>
              <p class="basket-card__mod">
                <?= esc($sel['group_label']) ?>: <?= esc($sel['choice']) ?>
                <?php if ($sel['price_delta'] > 0): ?>
                  <span class="mod-price">+<?= fmt($sel['price_delta']) ?></span>
                <?php endif; ?>
              </p>
            <?php endforeach; ?>
          </div>
    ...
    <div class="basket-summary">
      ...
      <a href="checkout.php" class="btn-primary" style="margin-top:16px;display:block;text-align:center">
        Proceed to checkout
      </a>
      <a href="index.php" class="btn-secondary" style="margin-top:8px;display:block;text-align:center">
        + Add more items
      </a>
    </div>
  </div>
